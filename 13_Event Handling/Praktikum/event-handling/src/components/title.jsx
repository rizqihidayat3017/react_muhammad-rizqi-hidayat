...
const Title = ({
    ...
    const TitleTodos = {
        ...
        fontWeight: 'bolder',
        marginBottom: '10px',
        backgroundImage: "linear-gradient(to right, #e9ed02 40%, #0cb80e 60%, #0320fd 50%)",
    ...
    return (
        <div>
            ...
            <form onSubmit={goSubmit} style={CustomForm}>
                <input type="text" style={CustomInput} value={goValue} onChange={goChange} placeholder="Add Todo..." />
            ...
            <ul style={CustomUl}>
                {items.map((item) =>
                    <div style={CustomContainerListItem}>
                        ...
                        <button style={CustomDelete} onClick={() => goDelete(item.id)}>Delete</button>
                    </div>
                )}
            </ul>
        </div>
